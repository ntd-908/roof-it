...
(function () {
    ...
    const toggleCaption = (image, data) => {
        // if (hasCaption(image)) {
        //     removeFigure(image);
        // } else {
        //     wrapInFigure(image);
    ...
    const create = function (normalizeCss, data) {
        ...
        if (data.caption === 'wrap') {
            let w = data.width ? data.width : 1,
                h = data.height ? data.height : 1,
                ratio = h * 100 / w,
                p = +(Math.round(ratio + "e+3") + "e-3");

            let figure = DOM.create('span', {class: 'img-native-wrap', style: "padding-bottom:" + p + "%"});
            figure.appendChild(image);
            return figure;
        } else {
            image.classList.add("img-fluid");
    ...
    const write = (normalizeCss, newData, image) => {
        ...
        updateProp(image, oldData, newData, 'caption', (image, _name, _value) => toggleCaption(image, newData));
    ...
    const splitTextBlock = (editor, figure) => {
        ...
        const textBlockElements = filter(editor.schema.getTextBlockElements(), (_, parentElm) => !editor.schema.isValidChild(parentElm, 'span'));
    ...
    const deleteImage = (editor, image) => {
        if (image) {
            ...
            const elm = editor.dom.is(image.parentNode, '.img-native-wrap') ? image.parentNode : image;
    ...
    const writeImageDataToSelection = (editor, data) => {
        ...
        if (image) {
            ...
            if (isFigure(image.parentNode)) {
                ...
                const w = data.width ? data.width : 1,
                    h = data.height ? data.height : 1,
                    ratio = h * 100 / w,
                    p = +(Math.round(ratio + "e+3") + "e-3");

                figure.setAttribute('style', "padding-bottom:" + p + "%");
                figure.setAttribute('data-mce-style', "padding-bottom:" + p + "%");
